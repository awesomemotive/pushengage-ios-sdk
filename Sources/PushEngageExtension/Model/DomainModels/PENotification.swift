...
@objc public class PENotification: NSObject {
    ...
    internal(set)public var isSponsored: Int

    @available(*, deprecated, renamed: "isSponsored")
    public var isSponsered: Int { isSponsored }
    ...
    package init(userInfo: [AnyHashable: Any]) {
        ...
        self.isSponsored = parsedValue?.custom?.isSponsered ?? 0
    ...
    package func timeOutTimerSetup() {
        // Strong capture is required: the timer is what keeps the notification
        // alive for the display window. completionTask invalidates the timer,
        // which releases the block and ends the retention.
        timeoutTimer = Timer(timeInterval: 25.0, repeats: false) { _ in
            self.completionTask(notification: self)
        }
    }

    package func setCompletion(for block: @escaping PENotificationDisplayNotification) {
        completionBlock = block
    }
    
    package func getCompletionBlock() -> PENotificationDisplayNotification {
    ...
    package func startTimeoutTimer() {
        if let unWrapTimeout = timeoutTimer {
            // Main run loop: the caller's thread (e.g. the UNUserNotificationCenter
            // delegate queue) has no spinning run loop, so a timer added there never fires.
            RunLoop.main.add(unWrapTimeout, forMode: .common)
        }
    }
...
@objc public class PENotificationOpenResult: NSObject {
    ...
    public init(notification: PENotification,
                notificationAction: PENotificationAction) {
        self.notification = notification
        self.notificationAction = notificationAction
        super.init()
    }

    @available(*, deprecated, renamed: "init(notification:notificationAction:)")
    public convenience init(notification: PENotification,
                            notficationAction: PENotificationAction) {
        self.init(notification: notification, notificationAction: notficationAction)
    }
}
