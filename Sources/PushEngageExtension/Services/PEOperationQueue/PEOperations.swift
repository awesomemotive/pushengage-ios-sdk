...
@available(iOS 10.0, *)
package typealias DownloadOperationInput = (attachmentString: String?,
...
package final class DownloadAttachmentOperation: ChainedAsyncResultOperation<DownloadOperationInput, String, PEError> {
    ...
    package override func main() {
        ...
        guard URL(string: attachmentString)?.scheme?.lowercased() == "https" else {
            PELogger.error(className: String(describing: DownloadAttachmentOperation.self),
                           message: "Attachment skipped: only https URLs are downloaded — \(attachmentString)")
            self.cancel()
            return
        }
    ...
    static func sanitizedAttachmentFilename(_ suggested: String?) -> String {
        ...
        if last.isEmpty || last == "." || last == ".." || last.contains("/") {
            return fallback
        }
        return last
    }
    ...
    private func updateAttactment(content: UNMutableNotificationContent,
        ...
        if let suggestedFilename = response.suggestedFilename {
            attachmentIdString = UUID().uuidString + Self.sanitizedAttachmentFilename(suggestedFilename)
        }
    ...
    package override final func cancel() {
...
@available(iOS 10.0, *)
package typealias SponseredNotificationInput = (previousAttachment: String?,
        ...
        input.notificationLifeCycle.withRetrysponseredNotification(with: input.notification) { [weak self] result in
            ...
            case .success(let response):
                ...
                if let sponseredID = response.sponseredActionButton?.first?.slabel {
                    ...
                    PECoreServices.getUserDefaults().setsponseredID(id: sponseredID)
                }
