...
package class Router: NetworkRouterType {
    ...
    private let taskLock = NSLock()   // guards `task`; critical sections must stay callout-free

    private func store(_ newTask: URLSessionTask?) {
        taskLock.lock()
        task = newTask
        taskLock.unlock()
    }

    private func loadTask() -> URLSessionTask? {
        taskLock.lock()
    ...
    package func request(_ route: PERouter,
        ...
        if NetworkConnectivity.isConnectedToInternet == false {
            ...
            do {
                ...
                let newTask = sessionManager.dataTask(with: request) { (data, response, error) in
    ...
    package func requestDownload(_ route: PERouter, completion: @escaping NetworkRouterDownloadCompletion) {
        ...
        if NetworkConnectivity.isConnectedToInternet == false {
            ...
            do {
                let request = try route.asURLRequest()
                PELogger.logNetworkRequest(className: String(describing: Router.self), request: request)
                let newTask = sessionManager.downloadTask(with: request) { (url, response, error ) in
                    if let error = error {
                        PELogger.error(className: String(describing: Router.self),
                                       message: error.localizedDescription)
                        completion(.failure(.networkError))
