...
internal struct TrackEventRequest: Codable {
    let siteId: Int
    ...
    let data: [String: TrackEventValue]
    ...
    /// Server-side defaults applied when callers don't override.
    static let defaultProvider  = "PushEngage"
    static let defaultEventType = "PushEngage.CustomEvent"
}
...
internal enum TrackEventValue: Encodable {
    case string(String)
    case int(Int64)
    case double(Double)
    case bool(Bool)
    ...
    static func from(_ value: Any) -> TrackEventValue? {
